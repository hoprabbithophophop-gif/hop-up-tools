  ...
  const fetchPickVideos = useCallback(async () => {
    ...
    for (const off of offsets) {
      const { data } = await supabase
        .from('youtube_videos')
        .select('video_id,title,channel_name,published_at,thumbnail_url,video_type,group_tags,description_short,duration_seconds,is_short')
        .eq('is_active_content', true)
        .not('channel_name', 'in', `(${GROUP_CHANNELS.join(',')})`)
        .gte('published_at', threeYearsAgo)
        .range(off, off);
    ...
    <div className="bg-white text-black min-h-screen">
      ...
      {sheetVideo && (
        <VideoChapterSheet
          ...
          playerTop={showPlayerAtTop ? 'calc(100vw * 9 / 16 + 72px)' : undefined}
          mode={{
            kind: 'add',
