...
interface Props {
  ...
  /** 原石の版のスマホ縦（案A・Hop決定 2026-09-26）: 動画が最上段にあるので、⚙・見出し・副題・案内・累計を全部動画の下に縦に並べる */
  stackBelow?: boolean;
}

export default function DiamondEntry({ total, gemColor, videoBottom, videoReady, loadingSlow, videoFailed = false, onRetry, onOpenSettings, reduceMotion = false, landscape = false, splitRight = null, stackBelow = false }: Props) {
  ...
  // 原石の版のスマホ縦（案A）: 動画が最上段にあるので、⚙も見出しも案内も累計も全部その下に縦に並べる
  if (stackBelow) {
    const top = videoBottom === null ? 240 : videoBottom + 10;
    return (
      <div style={{ position: "absolute", inset: 0, background: ARENA_BG }}>
        {__SHOW_VERSION__ && (
          <span style={{ position: "absolute", bottom: "calc(4px + env(safe-area-inset-bottom))", right: 8, fontSize: "0.5rem", color: "#6b7076", letterSpacing: "0.02em", pointerEvents: "none" }}>v.{__COMMIT_SHA__}</span>
        )}
        <div
          style={{
            position: "absolute", top, left: 0, right: 0,
            color: "#e8eaed", fontFamily: "Inter, 'Noto Sans JP', sans-serif",
            display: "flex", flexDirection: "column", alignItems: "center", gap: "0.7rem",
            padding: "0 1.2rem", textAlign: "center",
          }}
        >
          {onOpenSettings && (
            <button type="button" aria-label="表示設定" onClick={onOpenSettings} style={{ position: "absolute", top: -4, right: 10, zIndex: 2, background: "none", border: "none", fontSize: "1.25rem", lineHeight: 1, color: "#9aa0a6", cursor: "pointer", padding: "0.3rem" }}>⚙</button>
          )}
          <div>
            <h1 style={{ fontSize: "clamp(1.3rem, 6.5vw, 1.6rem)", fontWeight: 700, letterSpacing: "-0.02em", margin: 0, color: "#f5f7fa", lineHeight: 1.2 }}>灰toダイヤモンド</h1>
            <p style={{ fontSize: "0.8125rem", fontWeight: 600, letterSpacing: "0.04em", margin: "0.3rem 0 0", color: "#aab0b6" }}>{SUBTITLE_TAG}</p>
          </div>
          <p style={{ fontSize: "0.9375rem", fontWeight: 600, margin: 0, color: "#e8eaed", lineHeight: 1.5 }}>
            {videoReady
              ? "動画の再生ボタンを押すと はじまります"
              : videoFailed
          ...
          {!videoReady && videoFailed && onRetry && (
            <button type="button" onClick={onRetry} style={{ background: "#e8eaed", color: "#0b0d13", border: "none", padding: "0.55rem 1.4rem", fontSize: "0.9375rem", fontWeight: 700, cursor: "pointer" }}>もう一度</button>
          )}
          {shownTotal !== null && (
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span aria-hidden="true" style={{ display: "flex", flexShrink: 0 }}>
                <EntryGem size={GEM_SIZE} color={gemColor} animate={!reduceMotion} />
              </span>
