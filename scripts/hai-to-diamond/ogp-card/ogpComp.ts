...
import {
  ...
  setCalibration, useCalibration, PASS_MARK, BREAKS, WEIGHTS, gemOutline, outlineHitsBox, type Calib,
...
const H1_PX = 72;
const SUB_PX = 46;
/** 見出しと副題の間。見出しの大きさに対する割合なので、型ごとの文字の大きさで測り直す */
const gapFor = (h1: number) => ((h1 * 4.8) / 25.6) * 1.6;
const GAP_PX = gapFor(H1_PX); // 見出しと副題の間
const TEXT_PAD_LEFT = 80;
const GEM_ALPHA = 0.92; // 石を置くときの濃さ（いまのカードと同じ）
/** 【この写しで足した所】きれいに描くときの「中で何倍の細かさで描くか」。
 *  調整台は 2 で固定。大きい石で面の粗さが出るかを見比べるために外から変えられるようにした */
let SS_HI = 2;
/** 【この写しで足した所】きれいに描くときの石の絵の一辺の上限。
 *  調整台は 860 で頭打ちにしていて、それより大きい石は絵を引き伸ばして置いていた。
 *  構図1(780)と構図2(640)はどちらも 860 より小さいので、上げても絵は1画素も変わらない。
 *  大きい構図で面の粗さが出るかを見比べるために、外から上げられるようにした */
let SPRITE_MAX = 1300;
...
type LayoutDef = {
  ...
  text: {
    ...
    padTop: number;
    padBottom: number;
    /** 見出しの大きさ（px）。省略すると 72。副題は省略すると見出しの 46/72 */
    h1Px?: number;
    subPx?: number;
  };
  /** 石の大きさと置き場所 */
  gem: { sizePx: number; cx: number; cy: number };
  /** 【この写しで足した所】反対の色のにじみの置き場所。
   *  省略すると今までどおり動かさない（型1・型2は左上の 180,120 のまま）。
   *  構図3は石が左半分を埋めるので、にじみを左上に置くと石の裏に隠れてしまう */
...
/** 【この写しで足した所】構図3の「寄りの絵」用のにじみの置き場。
 *  石が左半分を埋めるので、にじみは右側の文字の無い所にだけ置く */
const C3_SLOTS: [number, number][] = [
  [780, 70], [960, 55], [1130, 95],
  [1150, 250], [960, 180], [1130, 380],
];

/** 3つの型。石の大きさと置き場所は、下の「置き場所の決め方」で測って決めた数。
 *
 *  【この写しで足した所】
 *  OGPの3構図（c1・c2・c3）。c1 は調整台の型1、c2 は型2 と1画素も違わない写し。
 *  c3 は新しく組んだ「寄りの絵」。型3（対角）は型1とほぼ同じ絵なので外した */
const LAYOUTS: LayoutDef[] = [
  ...
  {
    // 構図3 ＝ 寄りの絵。石を大きく取り、左と上下で断ち切る。文字は右に小さく添える。
    // 数は probe-ogp-comp.tmp.mjs で測って入れる（初めは当たりを付けた仮の値）
    key: "c3",
    // 文字の大きさと石の置き場所は probe-ogp-comp.tmp.mjs で 250通り引いて測った:
    //   文字に被らない 77.2%／出してよい姿 43.6%／そのうち尖りがカード内 58.7%／
    //   カードの中に見えている石の面積 69.0%
    text: { justify: "flex-end", align: "flex-end", textAlign: "right", padLeft: 0, padRight: 60, padTop: 0, padBottom: 44, h1Px: 56, subPx: 36 },
    gem: { sizePx: 1100, cx: 240, cy: 250 },
    comp: { x: 1020, y: 120 },
    mainOffset: { dx: MAIN_OFFSET.dx, dy: MAIN_OFFSET.dy, radius: MAIN_OFFSET.radius },
    slots: C3_SLOTS,
    // 石は左にあるので、右半分で測る
    amountArea: [{ x0: 600, y0: 0, x1: W, y1: H }],
    culetMustFit: true,
  },
  {
...
/** 【この写しで足した所】採点の目盛りの割り当て。
 *  c1 と c2 は型1・型2 と石の置き場所も文字の大きさも1画素も違わないので、
 *  調整台で測った目盛り（CALIB.l1 / CALIB.l2）をそのまま使う。
 *  c3 は新しい構図なので calib-ogp-comp.tmp.mjs で取り直したものを入れる。
 *  gemStudioScore.ts は読むだけで、書き換えていない */
/** 【この写しで足した所】構図3の採点の目盛り。
 *  calib-ogp-comp.tmp.mjs で 6000通り引き、文字に被る・尖りが上を向く・尖りがカードから出る
 *  を除いた 1407通りから作った（調整台の型1は 1662通り）。
 *  重みは型1・型3と同じ。合格の線は3条件の最低点の上位25%の位置 */
const C3_CALIB: Calib = {
  breaks: {
      brightness: [0.03137, 0.50707, 0.59896, 0.66583, 0.70677, 0.74589, 0.77710, 0.79876, 0.82533, 0.84810, 0.86924, 0.89102, 0.91196, 0.93293, 0.95551, 0.98295, 1.00000, 1.00000, 1.00000, 1.00000, 1.00000],
      scintillation: [0.01661, 0.10937, 0.13874, 0.15669, 0.17351, 0.18976, 0.20372, 0.21600, 0.23264, 0.24522, 0.25940, 0.27285, 0.28753, 0.29881, 0.31692, 0.33461, 0.35201, 0.37108, 0.40015, 0.43686, 0.66288],
      fire: [0.00951, 0.03756, 0.04696, 0.05465, 0.06158, 0.06763, 0.07271, 0.07888, 0.08405, 0.09045, 0.09624, 0.10155, 0.10805, 0.11370, 0.12119, 0.13091, 0.13806, 0.15040, 0.17045, 0.20300, 0.50952],
      identity: [0.73789, 0.88330, 0.90981, 0.92748, 0.93944, 0.95298, 0.96040, 0.96692, 0.97125, 0.97430, 0.97721, 0.97941, 0.98114, 0.98334, 0.98610, 0.98880, 0.99048, 0.99225, 0.99496, 0.99723, 1.00000],
      culet: [0.00000, 0.00000, 0.00000, 0.00000, 0.00000, 0.00000, 0.00000, 0.00000, 0.13342, 0.38774, 0.57828, 0.74731, 0.86335, 0.99705, 1.00000, 1.00000, 1.00000, 1.00000, 1.00000, 1.00000, 1.00000],
      culetDown: [0.00000, 0.06535, 0.12750, 0.19664, 0.27125, 0.33837, 0.39942, 0.45880, 0.52502, 0.58450, 0.63589, 0.68704, 0.73452, 0.77097, 0.82088, 0.85894, 0.89804, 0.92922, 0.95808, 0.98734, 0.99997],
  },
  weights: { brightness: 1, scintillation: 1, fire: 1, identity: 1, culet: 1, culetDown: 0.3 },
  pass: 0.5595,
};

let CALIB_C3: Calib | null = C3_CALIB;
function calibKeyOf(key: string): string {
  return key === "c1" ? "l1" : key === "c2" ? "l2" : key;
}
function useCalib(key: string) {
  if (key === "c3") {
    if (CALIB_C3) {
      setCalibration(CALIB_C3.breaks, CALIB_C3.weights, CALIB_C3.pass);
      return;
    }
    // まだ取り直していない間は型1の目盛りで動かす（測りの台本がここに入れる）
    useCalibration("l1");
    return;
  }
  useCalibration(calibKeyOf(key));
}
...
for (const d of LAYOUTS) {
  ...
  const k = 148 / W;
  const t = d.text;
  // 【この写しで直した所】文字の帯の大きさを、その型の文字の大きさに合わせて縮める
  const fk = (t.h1Px ?? H1_PX) / H1_PX;
  const headW = 580 * k * fk, headH = 86 * k * fk, subW = 421 * k * fk, subH = 55 * k * fk;
  const boxH = H * k;
  // 文字の帯の左上を、型の寄せ方から出す
  const tx = (w: number) =>
    t.align === "center" ? (148 - w) / 2 : t.align === "flex-end" ? 148 - w - t.padRight * k : t.padLeft * k;
  const blockH = headH + 21.6 * k * fk + subH;
  const ty =
  ...
  btn.innerHTML =
    ...
    `<i class="gs-mbar gs-msub" style="left:${tx(subW).toFixed(1)}px;top:${(ty + headH + 21.6 * k * fk).toFixed(1)}px;width:${subW.toFixed(1)}px;height:${subH.toFixed(1)}px"></i>` +
    `</span>`;
...
function importJson(raw: unknown) {
  ...
  ST = {
    ...
    layout: typeof o.layout === "string" && LAYOUT_BY_KEY.has(o.layout) ? o.layout : "c1",
    member: {
...
function applyLayoutText() {
  ...
  // 【この写しで足した所】文字の大きさも型ごとに入れ直す。
  // 大きさを変えると文字の箱の広さが変わるので、石が被る判定も明るさの比も
  // 自動でその型の文字で測り直される
  const h1 = t.h1Px ?? H1_PX;
  const sub = t.subPx ?? Math.round((h1 * SUB_PX) / H1_PX);
  (textsEl.querySelector("h1") as HTMLElement).style.fontSize = h1 + "px";
  subEl.style.fontSize = sub + "px";
  subEl.style.marginTop = gapFor(h1) + "px";
}
...
function applyLayout(key: string) {
  ...
  // 【この写しで足した所】反対の色のにじみの置き場所。型に指定があるときだけ入れ直す
  if (d.comp) { ST.glow.comp.x = d.comp.x; ST.glow.comp.y = d.comp.y; }
  else { const dd = makeDefault(); ST.glow.comp.x = dd.glow.comp.x; ST.glow.comp.y = dd.glow.comp.y; }
  applyLayoutText();
...
let spriteGen = 0;
/** 【この写しで足した所】きれいな絵を描き終えた回数。撮影の台本が描き終わりを待つのに使う */
let hiDraws = 0;

function drawGem() {
  const want = lowQuality
    ? Math.min(360, Math.max(96, Math.round(ST.gem.sizePx * 0.45)))
    : Math.min(SPRITE_MAX, Math.max(120, Math.round(ST.gem.sizePx)));
  const t0 = performance.now();
  const cv = renderGem({
    hex: ST.member.hex,
    px: want,
    ...
    half: fitHalf(),
    superSample: lowQuality ? 1 : SS_HI,
    sparkle: sparkleNow(),
  });
...
function draw() {
  ...
  seedEl.textContent = ST.shuffle.seed
    ...
    : "";
  if (!lowQuality) hiDraws++;
  (window as unknown as Record<string, unknown>).__compHiDraws = hiDraws;
  ...
  /** 【この写しで足した所】構図を外から測って詰めるための口。
   *  画面には出していない。撮影と測りの台本からだけ使う */
  (window as unknown as Record<string, unknown>).__compTune = {
    /** いまの型の石の大きさと置き場所を入れ替える（型の決めごとごと書き換える） */
    setGem: (cx: number, cy: number, size: number) => {
      const d = layoutDef();
      d.gem.cx = cx; d.gem.cy = cy; d.gem.sizePx = size;
      ST.gem.cx = cx; ST.gem.cy = cy; ST.gem.sizePx = size;
      ST.shuffle.baseCx = cx; ST.shuffle.baseCy = cy;
      ST.glow.main.x = cx + d.mainOffset.dx;
      ST.glow.main.y = cy + d.mainOffset.dy;
      ST.glow.main.radius = d.mainOffset.radius;
      syncAllControls();
      needGem = true;
      schedule();
    },
    /** いまの型の文字の置き方・大きさを入れ替える */
    setText: (o: Record<string, unknown>) => {
      const t = layoutDef().text as unknown as Record<string, unknown>;
      for (const k of Object.keys(o)) t[k] = o[k];
      applyLayoutText();
      needGem = true;
      schedule();
    },
    /** 尖りがカードの中に収まることを求めるかどうか */
    setCuletMustFit: (v: boolean) => { (layoutDef() as { culetMustFit: boolean }).culetMustFit = v; },
    /** 反対の色のにじみの置き場所 */
    setComp: (x: number, y: number) => {
      const d = layoutDef() as { comp?: { x: number; y: number } };
      d.comp = { x, y };
      ST.glow.comp.x = x; ST.glow.comp.y = y;
      needGem = true; schedule();
    },
    /** きれいに描くときの内部の細かさ（既定 2） */
    setSS: (n: number) => { SS_HI = Math.max(1, n); needGem = true; schedule(); },
    ss: () => SS_HI,
    /** 石の絵の一辺の上限（調整台の既定は 860） */
    setSpriteMax: (n: number) => { SPRITE_MAX = Math.max(120, n); needGem = true; schedule(); },
    spriteMax: () => SPRITE_MAX,
    /** きれいに描いた石の絵が、実際に何pxで描かれたか */
    lastPx: () => lastPx,
    /** 石の絵を描くのにかかった時間（ミリ秒） */
    lastMs: () => lastMs,
    /** 石の絵を作り直す（時間を測り直したいとき） */
    redrawGem: () => { needGem = true; schedule(); },
    /** 構図3の採点の目盛りを入れる */
    setCalib: (c: Calib) => {
      CALIB_C3 = c;
      if (ST.layout === "c3") useCalib("c3");
    },
    /** いまの型の決めごとを覗く */
    def: () => JSON.parse(JSON.stringify(layoutDef())),
  };
