/* =============================================================================
 * 【使い捨て・写し】src/pages/hai-to-diamond/gemRenderer.ts をそのまま写して、
 * 「角度と光の調整ツール」用に外からの口だけを広げたもの。本番の src/ は触っていない。
 *
 * 写しで広げた所（形・屈折・全反射・分散の計算は一字も変えていない）
 *   - 姿勢を、倒す角と回しの2つではなく 3x3 の回転そのもので受け取れるようにした
 *     （setPoseMatrix）。これでどの向きにも届く。
 *   - 3灯の向きと強さを外から渡せるようにした（applyLights）。灯りを動かすと
 *     部屋の絵（環境）も描き直して貼り直す。ここを直さないと閃光しか動かない。
 *   - 1枚だけ描いて絵を返す口を足した（renderGem）。
 *
...
/** 1灯ぶんの決めごと。dir は世界の向き（+X が画面右・+Y が画面上・+Z が手前） */
export type StudioLight = { dir: Vec3; intensity: number; radius: number };

/** きらめきの演出の決めごと。
 *  on を false にすると、部屋の絵も計算式も本番とまったく同じものに戻る */
export type Sparkle = {
  on: boolean;
  /** 部屋に足す、小さく鋭い光の数 */
  count: number;
  /** その光ひとつの大きさ。部屋の絵の横幅に対する割合 */
  radius: number;
  /** 散らし方 */
  seed: number;
  /** 石の中で光が跳ね返る回数の上限。4 が本番と同じ */
  bounces: number;
  /** 色の分かれ幅の倍率。1 が本物のダイヤの値。
   *  2 以上は物理から離れた【誇張】で、本物のダイヤはそこまで色が分かれない */
  disp: number;
  /** 全反射の境目に近い所で、3色の明るさの差を広げる強さ。
   *  0 で何もしない（その時は計算式の文字列にも一切出てこない） */
  split?: number;
  /** 光ひとつの明るさ。1 で真っ白。
   *  白すぎると、分かれた色が明るさで振り切れて白へ戻ってしまうので、
   *  少し下げて「鋭いけれど振り切らない」明るさにする */
  level: number;
};
/* 実測で決めた値。28個・大きさ0.010・明るさ0.80 が、
 * 火を 0.0730→0.0864 に上げつつ、白飛びを 0.0250 に留め、
 * その色だと分かる点の下がりを 0.991→0.966 に収められる組み合わせ。
 * これより小さい光は一切効かず（0.006 未満は数が1桁も動かない）、
 * 明るさ1.0まで上げると白飛びが5倍になる。跳ね返りは4のまま（6にすると3.3倍重い） */
/* 実測で決めた中身。
 * ・小さく鋭い光を足す案は取りやめた。大きさ0.006未満は数字が1桁も動かず、
 *   効く大きさまで上げると白飛びが5倍になり「その色だと分かる」が 0.991→0.962 に落ちるため。
 *   仕組みは残してあるので count を 0 より大きくすれば使える
 * ・代わりに、色の分かれ幅を本物の5倍に広げる。これは【誇張】で、
 *   本物のダイヤはここまで色が分かれない。切れば本物の値＝本番と同じ絵に戻る。
 *   実測: 白い石の火 0.0000→0.0304、黄の石 0.0012→0.0353、
 *   白飛びは変わらず、その色だと分かる点はむしろ上がり、描く時間も変わらない */
/* ・さらに、全反射の境目に近い角度で出ていく光ほど3色の向きが大きく開くことを使い、
 *   その開きが大きい所だけ色の差を広げる（split）。これも【誇張】。
 *   実測（780pxで、色の付いた塊を数えた）: 強め無し→×2 で
 *     nishida 36→56枚、hirai 23→49枚、kojima 16→47枚、eguchi 28→63枚。
 *   白飛びはむしろ減り（nishida 0.0096→0.0001、hirai 0.0308→0.0008）、
 *   描く時間も変わらない（780px を描いて読み出すまで 23.4→21.8ミリ秒）。
 *   ×4 は採らなかった。増える枚数はわずか（nishida 56→62枚）なのに、
 *   その色だと分かるかが nishida 0.994→0.971・hirai 0.985→0.967 と
 *   ×1→×2 の下がり（0.003〜0.004）の5倍以上落ちるため */
export const DEFAULT_SPARKLE: Sparkle = { on: true, count: 0, radius: 0.01, seed: 7, bounces: 4, level: 0.8, disp: 5, split: 2 };
export const PLAIN_SPARKLE: Sparkle = { on: false, count: 0, radius: 0, seed: 0, bounces: BOUNCES, level: 0, disp: 1, split: 0 };

function sparkleKey(sp: Sparkle): string {
  return sp.on ? [sp.count, sp.radius.toFixed(4), sp.seed, sp.level.toFixed(3), sp.disp.toFixed(3), (sp.split ?? 0).toFixed(3)].join(",") : "off";
}
/** 同じ種からは同じ散らし方になる小さな数当て器 */
function spRng(seed: number): () => number {
  let a = (seed >>> 0) || 1;
  return () => {
    a = (a + 0x6d2b79f5) >>> 0;
    let t = a;
...
/** 本番がいま使っている3灯。調整ツールの初期値はこれ */
export function defaultLights(): StudioLight[] {
...
function keyLights(): StudioLight[] {
  return defaultLights();
}

/** 灯りの組が変わったかどうかを見分けるための合言葉 */
function lightsKey(lights: StudioLight[]): string {
  return lights
    .map((l) => l.dir.map((v) => v.toFixed(5)).join(",") + "|" + l.intensity.toFixed(4) + "|" + l.radius.toFixed(4))
    .join(";");
}
...
let envCanvasKey = "";
function buildEnvCanvas(lights: StudioLight[] = defaultLights(), sparkle: Sparkle = PLAIN_SPARKLE): HTMLCanvasElement {
  const wantKey = lightsKey(lights) + "#" + sparkleKey(sparkle);
  if (envCanvas && envCanvasKey === wantKey) return envCanvas;
  ...
  // 主光源（3灯）を、方向から求めた位置に置く。どこに置くかは引数で受け取る
  ...
  // 小さく鋭い光（宝石店の細かいスポットライトに当たるもの）。
  // 大きくやわらかい光だけだと、虹に分かれた色が隣どうしで混ざって白へ戻ってしまう。
  // 粒が小さいほど、赤・緑・青が別々の方向へ抜けた時に片方だけが光を拾う
  if (sparkle.on && sparkle.count > 0) {
    const r = spRng(sparkle.seed);
    for (let i = 0; i < sparkle.count; i++) {
      const u = r();
      // 上寄りに置く。真上から斜め上あたりまで
      const v = 0.04 + r() * 0.52;
      // 一度だけ塗る。重ね塗りすると中心が真っ白に振り切れて、色が白へ戻る
      paintBlob(ctx, u, v, sparkle.radius, sparkle.level);
    }
  }

  ctx.globalCompositeOperation = "source-over";
  envCanvas = cv;
  envCanvasKey = wantKey;
  return cv;
}
...
function fragmentSource(nPlanes: number, bounces: number = BOUNCES, disp: number = 1, split = 0): string {
  // disp が 1 のときは、本番とまったく同じ文字列になるようにそのまま使う
  const iorR = disp === 1 ? IOR_R : IOR_G + (IOR_R - IOR_G) * disp;
  const iorB = disp === 1 ? IOR_B : IOR_G + (IOR_B - IOR_G) * disp;
  return [
    ...
    "varying vec3 vNrmObj;",
    "",
    "const float IOR_R = " + iorR.toFixed(4) + ";",
    "const float IOR_G = " + IOR_G.toFixed(4) + ";",
    "const float IOR_B = " + iorB.toFixed(4) + ";",
    ...
    ...(split > 0
      ? [
          // 色の開きをどれだけ強めるか。SPLIT_REF は「これだけ開いたら目いっぱい」の角度
          "const float SPLIT_GAIN = " + split.toFixed(4) + ";",
          "const float SPLIT_REF = 0.1500;",
        ]
      : []),
    ...
    // 全反射の境目に近い角度で出ていく光ほど、赤・緑・青の出る向きが大きく開く。
    // その開きが大きい所だけ、3色の明るさの差を広げる。これは物理ではなく【誇張】。
    //
    // 広げるのは「石の吸収を通したあとの光」に対して行う。吸収の前に広げると、
    // 石が飲みこんでいるはずの色まで持ち上がって、桃色の石に緑の面が出てしまう。
    // 通したあとで広げれば、平均より暗い色（＝その石が吸っている色）はさらに
    // 下がるだけなので、石が通さない色は出てこない
    ...(split > 0
      ? [
          "        vec3 lit = col * absorbT;",
          "        if (dot(oR, oR) > 0.0 && dot(oB, oB) > 0.0) {",
          "          float opn = acos(clamp(dot(normalize(oR), normalize(oB)), -1.0, 1.0));",
          "          float sp = clamp(opn / SPLIT_REF, 0.0, 1.0);",
          "          float mid = (lit.r + lit.g + lit.b) / 3.0;",
          "          lit = max(vec3(0.0), mid + (lit - mid) * (1.0 + SPLIT_GAIN * sp));",
          "        }",
          "        body += through * (1.0 - Rf) * lit;",
        ]
...
type Gear = {
  ...
  /** 部屋の絵を貼り付けてある場所。灯りを動かすたびに貼り直す */
  tex: WebGLTexture | null;
  /** いま貼ってある部屋の絵が、どの灯りの組で描かれたものか */
  envKey: string;
  /** 演出用の計算式の控え。跳ね返りの回数と色の分かれ幅の組ごとに持つ */
  variants: Map<string, { prog: WebGLProgram; loc: Loc }>;
  /** いまどちらの計算式を使っているか */
  usingHi: boolean;
...
function buildGear(): Gear | null {
  ...
  try {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, buildEnvCanvas(defaultLights()));
    ...
    gear = {
      ...
      tex: tex,
      envKey: lightsKey(defaultLights()) + "#off",
      variants: new Map(),
      usingHi: false,
      planeData: planeData,
    };
...
/** 石の姿勢を、3x3 の回転（石の座標 → 世界の座標）で直に入れる。
 *  並べ方は行の順（r00, r01, r02, r10, r11, r12, r20, r21, r22）。
 *  上の setPose が組む並びと同じ意味なので、倒す角と回しで作った回転を
 *  そのまま渡せば、一字も違わず同じ絵になる。こちらはどの向きにも届く。 */
function setPoseMatrix(g: Gear, r: ArrayLike<number>, cx: number, cy: number) {
...
/** 跳ね返りの回数を増やした2本目の計算式を、必要になった時だけ組む。
 *  組み上げは1回だけで、あとは使い回す */
function ensureHi(g: Gear, bounces: number, disp: number, split = 0): { prog: WebGLProgram; loc: Loc } | null {
  if (bounces <= BOUNCES && disp === 1 && split <= 0) return null;
  const key = bounces + "|" + disp.toFixed(3) + "|" + split.toFixed(3);
  const hit = g.variants.get(key);
  if (hit) return hit;
  try {
    const gl = g.gl;
    const nPlanes = g.planeData.length / 4;
    const prog = buildProgram(gl, VERT, fragmentSource(nPlanes, bounces, disp, split));
    const loc: Loc = {
      ...
      uPlanes: gl.getUniformLocation(prog, "uPlanes[0]"),
      uEnv: gl.getUniformLocation(prog, "uEnv"),
      uO2W: gl.getUniformLocation(prog, "uO2W"),
      uViewObj: gl.getUniformLocation(prog, "uViewObj"),
      uAbsorb: gl.getUniformLocation(prog, "uAbsorb"),
      uHue: gl.getUniformLocation(prog, "uHue"),
      uKeyDir: gl.getUniformLocation(prog, "uKeyDir[0]"),
      uKeyInt: gl.getUniformLocation(prog, "uKeyInt[0]"),
    };
    // 形を渡す口は計算式ごとに別なので、組んだ直後に一度入れておく
    gl.useProgram(prog);
    gl.uniform4fv(loc.uPlanes, g.planeData);
    gl.uniform1i(loc.uEnv, 0);
    gl.enableVertexAttribArray(loc.position);
    gl.enableVertexAttribArray(loc.normal);
    g.envKey = "";  // 灯りは入れ直しになる
    const made = { prog, loc };
    g.variants.set(key, made);
    return made;
  } catch (e) {
    console.warn("[調整ツール] 演出用の計算式を組めなかった:", e);
    return null;
  }
}

/** いま使う計算式と、その口の組を返す */
function pick(g: Gear, v: { prog: WebGLProgram; loc: Loc } | null): Loc {
  g.gl.useProgram(v ? v.prog : g.program);
  if (g.usingHi !== !!v) {
    g.usingHi = !!v;
    g.envKey = "";  // 計算式が変わったら灯りを入れ直す
  }
  return v ? v.loc : g.loc;
}

/** 灯りの組を入れ直す。部屋の絵も描き直して貼り直すので、
 *  灯りを動かすと閃光だけでなく面が拾う光そのものが変わる */
function applyLights(g: Gear, lights: StudioLight[], sparkle: Sparkle, loc: Loc) {
  const key = lightsKey(lights) + "#" + sparkleKey(sparkle) + "#" + (g.usingHi ? "hi" : "lo");
  if (g.envKey === key) return;
  const gl = g.gl;
  gl.activeTexture(gl.TEXTURE0);
  ...
  gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, false);
  gl.pixelStorei(gl.UNPACK_COLORSPACE_CONVERSION_WEBGL, gl.NONE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, buildEnvCanvas(lights, sparkle));
  const keyDir = new Float32Array(3 * 3);
  const keyInt = new Float32Array(3);
  for (let k = 0; k < 3; k++) {
    const l = lights[k] ?? { dir: [0, 0, 1] as Vec3, intensity: 0, radius: 0.1 };
    const d = norm(l.dir);
    keyDir[k * 3 + 0] = d[0];
    keyDir[k * 3 + 1] = d[1];
    keyDir[k * 3 + 2] = d[2];
    keyInt[k] = l.intensity;
  }
  gl.uniform1i(loc.uEnv, 0);
  gl.uniform3fv(loc.uKeyDir, keyDir);
  gl.uniform1fv(loc.uKeyInt, keyInt);
...
class Baker implements BakeJob {
  ...
  private arm(g: Gear) {
    const gl = g.gl;
    pick(g, null);
    applyLights(g, defaultLights(), PLAIN_SPARKLE, g.loc);
...
/** 石の角の点をそのまま返す。石の影が文字に掛かるかを、
 *  絵を描かずに形だけで見分けるために使う */
export function gemVertices(): number[][] {
  const g = buildGear();
  if (!g) return [];
  return g.verts.map((v) => [v[0], v[1], v[2]]);
}

/** 石を原点まわりにすっぽり包む球の半径。
 *  どの向きに回しても見かけの大きさが変わらない枠を作るのに使う */
export function gemBoundRadius(): number {
  const g = buildGear();
  if (!g) return 1;
  let m = 0;
...
/** 倒す角と回しの組に対して、本番が使うのと同じ枠を返す。
 *  写しと本番が同じ絵になるかを確かめるときに使う */
export function poseFitBounds(poses: Pose[]): { cx: number; cy: number; half: number } | null {
  const g = buildGear();
  if (!g) return null;
  return screenBounds(g.verts, poses);
}

/** 外形が枠の何割を占めるか（本番と同じ値） */
export const GEM_FIT = FIT;

export type GemRenderOptions = {
  /** 石の色（#rrggbb）。白は無色のダイヤになる */
  hex: string;
  /** 出来上がる絵の一辺（px） */
  px: number;
  /** 石の座標 → 世界の座標の回転。行の順に9個 */
  rot: ArrayLike<number>;
  /** 3灯。省略すると本番と同じ既定の3灯 */
  lights?: StudioLight[];
  /** 枠の半分の大きさ。省略すると、どの向きでも見かけが変わらない大きさ */
  half?: number;
  /** 枠の中心をずらす量。省略すると石の中心（原点） */
  cx?: number;
  cy?: number;
  /** 内部で何倍の細かさで描くか。省略すると本番と同じ 2 倍 */
  superSample?: number;
  /** 描き込む先の絵。省略すると新しく作る */
  target?: HTMLCanvasElement;
  /** きらめきの演出。省略すると演出なし＝本番とまったく同じ描き方 */
  sparkle?: Sparkle;
};

/** 1枚だけ描いて絵を返す。使えない端末では null を返す（例外は投げない） */
export function renderGem(o: GemRenderOptions): HTMLCanvasElement | null {
  const g = buildGear();
  if (!g) return null;
  try {
    const gl = g.gl;
    const sparkle = o.sparkle ?? PLAIN_SPARKLE;
    const variant = sparkle.on ? ensureHi(g, sparkle.bounces, sparkle.disp, sparkle.split ?? 0) : null;
    const loc = pick(g, variant);
    applyLights(g, o.lights ?? defaultLights(), sparkle, loc);

    const half = o.half ?? gemBoundRadius() / FIT;
    gl.uniformMatrix4fv(loc.projectionMatrix, false, setOrtho(g.proj, half));

    const px = Math.max(8, Math.round(o.px));
    const ss = Math.max(1, o.superSample ?? SUPERSAMPLE);
    const inner = Math.max(8, Math.round(px * ss));
    if (g.size !== inner) {
      g.canvas.width = inner;
      g.canvas.height = inner;
      g.size = inner;
    ...
    const col = hexToAbsorb(o.hex);
    gl.uniform3f(loc.uAbsorb, col.absorb[0], col.absorb[1], col.absorb[2]);
    gl.uniform3f(loc.uHue, col.hue[0], col.hue[1], col.hue[2]);

    setPoseMatrix(g, o.rot, o.cx ?? 0, o.cy ?? 0);
    gl.uniformMatrix4fv(loc.modelViewMatrix, false, g.mv);
    gl.uniformMatrix3fv(loc.uO2W, false, g.o2w);
    gl.uniform3f(loc.uViewObj, g.viewObj[0], g.viewObj[1], g.viewObj[2]);

    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
    gl.drawArrays(gl.TRIANGLES, 0, g.count);

    const cv = o.target ?? makeCanvas(px);
    ...
    ctx.imageSmoothingEnabled = true;
    if ("imageSmoothingQuality" in ctx) ctx.imageSmoothingQuality = "high";
    ctx.drawImage(g.canvas, 0, 0, px, px);
    return cv;
  } catch (e) {
    console.warn("[調整ツール] 石を描けなかった:", e);
    return null;
  }
}
